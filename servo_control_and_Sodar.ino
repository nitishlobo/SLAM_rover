...
/*
 * angle 98 is then sonar is straight ahead
 */

 const byte servo_pin = 5;
 const byte sonar_trig = 2;
 const byte sonar_echo = 3;
...
// Sonar CODE:
/*----------------------------------------------------------------*/
 int sonar_ping(){      //returns the distance of the ping in cm
  int dist;
  dist = sonar.ping_cm();
  return dist;
 }

int ave_sonar_ping(int number_of_pings) {   //returns the average distance ofer set number of pings
  int t = sonar.ping_median(number_of_pings);
  int dist = sonar.convert_cm(t);
  return dist;
}
...
/*----------------------------------------------------------------*/
boolean can_ping() {    //checks if enoguh time has passed for acurate pinging. Should be rn before any other servo/sonar code
